...
- **🎚 Vocal & audio transforms** — open the per-segment transform modal to reshape an existing clip's audio (voice **or** foley): pitch / formant, creative colours (sub-octave, drive, ring-mod, vibrato), space effects (**🧱 Echo** slap-back/alley-bounce and **🕳 Reverb** room→cave, which add a decaying tail past the clip end), presets (Vader, Robot, Alley, Hall, Cave, Slapback…), and a **☎️ Telephone** lo-fi "bad phone call" effect (band-limit + crackle). Preview before committing; the bake is reversible (re-open and remove to restore the original).
